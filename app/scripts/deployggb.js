...
//        setTimeout(validateJavaApplet(appletElem, container_ID),5000);
  ...
  var injectHTML5Applet = function(appletElem, parameters, noPreview) {
    ...
    if (!noPreview && parameters.width !== undefined) {
        ...
        parameters.appletOnLoad = function() {
          ...
          for (key in parameters) {
            if (parameters.hasOwnProperty(key)) {
              article.removeAttribute("data-param-" + key); // by rhcad
            }
          }
      ...
      // Resize after the applet loaded. Added by rhcad
      if (typeof parameters.appletOnLoad === "function") {
        var oriAppletOnload2 = parameters.appletOnLoad;
      }
      parameters.appletOnLoad = function() {
        if (typeof oriAppletOnload2 === "function") {
          oriAppletOnload2();
        }
        applet.resize();
      };
...
//            appletScaler.appendChild(resource3);
  ...
  var setHTML5CodebaseInternal = function(codebase, offline) {
    ...
    if (/^test/.test(html5CodebaseVersion)) {
      html5CodebaseVersion = html5CodebaseVersion.substring(4, 1) + '.' + html5CodebaseVersion.substring(5, 1);
    } else if (/^(war|beta)/.test(html5CodebaseVersion)) {
...
var GGBAppletUtils = (function() {
  ...
  function getWidthHeight(appletElem, appletWidth, allowUpscale, noBorder) {
    ...
    if (container) {
      ...
      if (appletElem) {
        var appletParent = appletElem.parentElement;
        var rect = appletElem.getBoundingClientRect();
        if (appletParent && appletParent.id === 'applet-html') {  // by rhcad
          rect = appletParent.getBoundingClientRect();
        }
        if (rect.left > 0) {
  ...
  function responsiveResize(appletElem, parameters) {
    ...
    if (article) {
      ...
      if (scaleElem !== null) {
        ...
        scaleElem.parentNode.style.transform = "";
        if (!isNaN(scale) && scale !== 1) {
          // Set the scale factor for the applet. Fix by rhcad
          scaleElem.style.transform = "scale(" + scale + "," + scale + ")";
          scaleElem.parentNode.style.width = ((parameters.width || article.offsetWidth) * scale) + 'px';
          scaleElem.parentNode.style.height = ((parameters.height || article.offsetHeight) * scale) + 'px';
        } else {
          // Remove scaling
          scaleElem.style.transform = "none";
